...
| `drive_layout.driven_wheels`             | 可输出驱动力矩的车轮集合   | 必须与 `tau_drv_*` 字段和动力分配逻辑一致                                     |
...
## 4. Teacher Simulator 设计

teacher simulator 的详细设计见 `refine-logs/TEACHER_SIMULATOR_DESIGN.md`。本文档只保留数据生成侧需要依赖的范围摘要。

teacher simulator 的保真度应高于 student model，第一版采用可实现的高保真近似：

```text
车身:
  6-DOF chassis dynamics
...
执行器:
  steering ratio / lag / saturation
  drive/brake torque limits
  throttle/brake delay
...
```text
teacher 内部真实参数不进入 student input
teacher-only 字段只用于 auxiliary loss / diagnostics / oracle ablation
所有 scenario、metadata、observable、teacher_only 字段必须能覆盖本文件 DS0/DS1 设计
```
...
sw_angle
tau_drv_fl, tau_drv_fr, tau_drv_rl, tau_drv_rr
tau_brk_fl, tau_brk_fr, tau_brk_rl, tau_brk_rr
...
| `tau_drv_fl/fr/rl/rr` | 四轮驱动力矩观测或估计 | 如果真实车无法提供，需要后续 actuator inference |
| `tau_brk_fl/fr/rl/rr` | 四轮制动力矩观测或估计 | 如果真实车无法提供，需要后续 brake allocation / inference |
| `steer_cmd` | 转向执行器命令 | 用于建模执行器 delay / saturation |
| `throttle_cmd` | 加速踏板或驱动请求 | 与 `tau_drv_*` 一起用于驱动输入一致性检查 |
| `brake_cmd` | 制动踏板或制动请求 | 与 `tau_brk_*` 一起用于制动输入一致性检查 |

如果真实车无法提供 `tau_drv_i / tau_brk_i`，需要在后续版本加入 torque allocation / actuator inference 模块；第一版仿真数据优先保证四轮力矩可观测或可导出。
...
约定左转为正方向，右转为负方向。具体 steering waveform 在第 8 章定义，可使用 step、sine、sweep 或 lane-change 形式。
...
不要按工况组合数量直接采样。第 7.6 节中 `CG-SINGLE:CG-SPLIT:CG-TRANSITION = 100:300:300` 只是 scenario space 大小，不应直接变成训练数据比例。否则 split 和 transition 会占训练集的 `6/7`，基础单一路况不足。
...
控制输入脚本用于实例化第 7 章的 `L* × Y*` 控制因子。每个 scenario 至少绑定一个 script；DS1 中可对同一 `scenario_id` 采样多个 amplitude / duration / seed，用于构造 held-out control amplitude。
...
DS1 可以生成第 7 章定义的完整工况组合；held-out road μ 是 split 规则，表示部分 road factor、transition、Split-μ 组合只进入 validation/test，不进入对应训练 split。

### 9.3 Held-Out Vehicle / Config
...
### 9.4 Held-Out Target Time Windows
...
```text
vehicle_internal_params_hash 用于追踪真实质量/惯量/悬架/轮胎等内部随机化
