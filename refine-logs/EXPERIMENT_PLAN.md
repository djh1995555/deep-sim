...
2. 通过单因素 ablation 判断 E/T/F/S/M/V/U 各组件是否值得保留
...
- **统一协议**：
  ...
  - 每次只替换 base 中 E/T/F/S/M/V/U 的一项，其余模块保持 `E1 + T1 + F1 + S1 + M1a + V1 + U0`；
...
- **固定条件**：
  - 其余模块保持 base：`E1 + F1 + S1 + M1a + V1 + U0`；
  - encoder、MuHead、FzResidualNN、VehicleResidualNN 和 uncertainty 设置不变。
- **成功标准**：
...
- **固定条件**：
  - 其余模块保持 base：`E1 + T1 + S1 + M1a + V1 + U0`；
...
- **对照系统**：
  ...
  - `S1`：`S0 + steerResidualNN`。
- **固定条件**：
  - 其余模块保持 base：`E1 + T1 + F1 + M1a + V1 + U0`；
...
#### B4.5：Shared Encoder Ablation

- **目的**：比较三种 history encoder 对路面 latent、执行器滞后、载荷/轮胎 residual 和长 rollout 的影响，决定第一版默认 encoder。
- **对照系统**：
  - `E1`：small GRU encoder，base 默认；
  - `E2`：causal TCN encoder；
  - `E3`：causal Transformer encoder。
- **固定条件**：
  - 其余模块保持 base：`T1 + F1 + S1 + M1a + V1 + U0`；
  - 三者使用相同 observable history、`fixed_vehicle_context`、`nominal_physics_prior`；
  - `z_shared` 维度固定为 128；
  - history_len、rollout horizon、训练预算、optimizer 和 split 保持一致；
  - 所有 encoder 必须 causal，禁止未来帧泄漏。
- **指标**：
  - rollout RMSE：`vx/vy/r/roll/pitch/omega_i`；
  - low-μ / Split-μ / transition 场景的 `vy/r/omega_i` 和 yaw drift；
  - `MuHead` calibration、transition response delay 和 small-slip uncertainty；
  - downstream residual magnitude：`FzResidualNN/TireResidualNN/VehicleResidualNN`；
  - 训练稳定性、推理延迟、显存和参数量；
  - held-out vehicle/config 泛化。
- **成功标准**：
  - `E2` 或 `E3` 只有在 transition、Split-μ、长 rollout 或 held-out vehicle/config 上稳定优于 `E1`，且物理约束和 residual magnitude 不退化时，才替代 `E1`；
  - 如果 `E2/E3` 仅小幅降低 train/val loss，但推理成本、泛化或 residual 归因变差，则不进入第一版默认；
  - 如果三者效果接近，优先保留 `E1`，因为结构更小、在线递推和 fine-tune 更简单。
- **后续动作**：
  - 若最终 encoder 不是 `E1`，需要用最终 encoder 重跑关键组件对照中的最小集合：`T0/T1/T2`、`M0-fixed/M1a/M1b`、`V0/V1`。

#### B4.6：Uncertainty / Ensemble Ablation
...
- **固定条件**：
  - 其余模块保持 base：`E1 + T1 + F1 + S1 + M1a + V1`；
...
#### B4.7：`VehicleResidualNN` 容量与位置 Ablation
...
- **固定条件**：
  - 其余模块保持 base：`E1 + T1 + F1 + S1 + M1a + U0`；
...
5. `E1/E2/E3`：比较 history encoder；
6. `V0/V1/V1-large/V2-small`：确认 final residual 不退化；
7. `U0/U1`：最后做 ensemble，因为成本最高，且依赖单模型稳定。
- **退出条件**：
  - 每个增强模块都能证明收益，或被明确淘汰；
