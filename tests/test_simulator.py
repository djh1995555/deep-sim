...
class TeacherSimulatorSmokeTest(unittest.TestCase):
    ...
    def test_simulation_app_exports_single_episode_dataset(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("vehicle.beta_rad", rows[0])
            self.assertIn("vehicle.beta_deg", rows[0])
            self.assertIn("vehicle.slip_angle_fl", rows[0])
            self.assertIn("vehicle.slip_angle_fr", rows[0])
            self.assertIn("vehicle.slip_angle_rl", rows[0])
            self.assertIn("vehicle.slip_angle_rr", rows[0])
            self.assertIn("vehicle.Fy_true_fl", rows[0])
            self.assertIn("vehicle.Fz_true_fl", rows[0])
            self.assertIn("vehicle.friction_usage_fl", rows[0])
            self.assertIn("vehicle.delta_eff_fl", rows[0])
            report = TeacherEpisodeValidator().validate_dataset(tmp)
            self.assertTrue(report.passed, report.errors)
