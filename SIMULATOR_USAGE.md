...
| `simulator/controller/coupled_mpc.py`            | 横纵向耦合 MPC 控制器，联合优化加速度和方向盘角，当前使用无外部求解器的 finite-horizon shooting。                 |
...
| `controller_type`             | string      | 对外 controller facade 选择的控制器类型。                                                                                                  | 默认 `modular`，即纵向 PID + 横向 LQR；可设为 `coupled_mpc`。        |
...
| `mpc_config`                  | mapping     | `controller_type: coupled_mpc` 时使用的 MPC 参数。                                                                                         | 见下方 MPC 字段表。                                                 |
...
| `prediction_dt` | MPC 内部预测步长，单位秒；可以大于车辆模型 `dt_internal`。 |
| `accel_min_mps2` / `accel_max_mps2` | MPC 可选择的纵向加速度范围。 |
| `accel_span_mps2` / `accel_samples` | 围绕当前速度误差中心生成加速度候选的范围和数量。 |
| `max_sw_angle_rad` | MPC 输出方向盘角饱和上限。 |
| `steering_span_rad` / `steering_samples` | 围绕前馈 + 反馈中心生成方向盘角候选的范围和数量。 |
| `speed_weight` | 速度误差权重。 |
| `lateral_weight` | 横向误差权重。 |
| `heading_weight` | 航向误差权重。 |
| `longitudinal_weight` | 沿 reference 方向的位置误差权重。 |
| `accel_effort_weight` / `steering_effort_weight` | 控制量大小惩罚。 |
| `accel_rate_weight` / `steering_rate_weight` | 相邻控制命令变化惩罚。 |
| `friction_excess_weight` | 预测横纵向合加速度超过当前路面 μ 极限时的惩罚。 |
| `terminal_weight` | horizon 末端状态额外权重。 |
| `lateral_feedback_gain` / `heading_feedback_gain` | 用于生成 steering 搜索中心的横向 / 航向反馈增益。 |
| `speed_feedback_gain` | 用于生成 accel 搜索中心的速度误差反馈增益。 |
| `throttle_feedforward` / `accel_to_throttle` / `decel_to_brake` | 将 MPC 选择的加速度映射为 throttle / brake 命令的参数。 |
| `friction_safety_factor` | 附着极限安全系数，越小越保守。 |
...
默认 CLI 当前使用 `_default_controller()` 构建 `SimulationController`。`SimulationController` 是对外 facade，内部可以通过 `controller_type` 切换控制器。当前支持：

| `controller_type` | 内部控制器 |
| --- | --- |
| `modular` | 纵向 PID + 横向 LQR，保持历史默认行为。 |
| `coupled_mpc` | 横纵向耦合 MPC，联合选择加速度和方向盘角。 |
...
| `controller/coupled_mpc.py` | 横纵向耦合 MPC，基于简化自行车模型做 finite-horizon shooting，不依赖外部求解器。 |
