...
| `configs/`            | 实验配置                 | 数据集生成配置、仿真 request 和每个 run 的 YAML 配置                                      | 定义实验入口参数，是复现实验的主要配置来源。                                                                   |
...
| `output/`             | 运行输出根目录              | `training/` 和 `simulation/` 两类输出                                                   | 所有新运行产物统一写入这里，避免重新引入 `runs/`。                                                             |
...
数据集生成配置。旧的 `configs/teacher/` 已移除；这些 YAML 现在由 `simulator.data_generator` 和 `simulator.vehicle_model` 使用。

| 文件 | 用途 |
| --- | --- |
| `ds0_minimal.yaml` | 最小仿真数据集配置，用于早期 sanity、data generator 和测试。 |
...
| `ds1_proxy_ft_v1.yaml` | target-window fine-tune proxy 数据配置，用于 FT0-FT6 相关实验。 |
| `ds2_extreme_v0.yaml` | DS2 极限操控 scaffold 数据配置，用于 B7 / MoE tire residual 后续实验入口。 |

### `configs/simulator/`

单次闭环仿真器 request 配置。它不负责批量采样数据集，而是在 YAML 内显式指定 `model` 数值配置、`scenario` 场景集合、控制器参数后，输出一个 canonical episode 与 debug trace。`scenario.reference` 通常指向 `configs/simulator/references/` 下的 reference 文件。
...
| 子目录 | 作用 |
| --- | --- |
| `b0_data_generation/` | B0 数据生成和 simulator/data_generator sanity。 |
...
注意：`data/` 是稳定输入目录；新实验的输出不要写进这里，应写入 `output/training/` 或 `output/simulation/`。
...
| `vehicle_model/` | 高保真车辆物理模型，包括配置、工况、状态、导出、验证和物理子模块。 |
| `data_generator/` | 批量训练数据生成入口，调用 `vehicle_model` 生成 DS0/DS1/DS1 proxy/DS2 数据。 |
| `visualizer/` | debug trace 输出与可视化接口，当前支持 JSON/CSV、Plotly HTML debug report，以及从既有 trace 重新生成 HTML 的 CLI。 |
...
| `output/simulation/` | `simulator.cli` 单次闭环仿真输出、debug trace、Plotly HTML、canonical episode | 手动闭环仿真和可视化结果的默认位置。 |
...
单次闭环仿真和 debug 可视化结果目录。每个仿真 request 的 `out_dir` 是输出目录模板，默认每次运行会把时间戳插入到父路径和目录名之间，例如 `output/simulation/20260522_145312_123456/double_lane_change_5mps/`，避免重跑时覆盖旧结果。

典型内容：

| 文件/目录 | 用途 |
| --- | --- |
| `manifest.json` | 单次仿真生成的 canonical dataset manifest。 |
| `episodes/*.npz`、`episodes/*.json` | episode 数值数组和 sidecar metadata。 |
| `simulation_request.json` | 本次仿真的解析后 request。 |
| `simulation_summary.json` | 仿真输出摘要。 |
| `debug_trace.json`、`debug_trace.csv` | controller / vehicle_model 通信变量和 debug 信号。 |
| `debug_report.html` | Plotly 交互式可视化报告。 |
...
2. `configs/experiments/` 和 `configs/datasets/`：实验是否可复现。
...
3. 新生成的 `output/training/*` 和 `output/simulation/*` 产物：除非明确是在清理、归档或重新生成实验结果。
