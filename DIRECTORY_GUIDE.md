...
| 文件 | 用途 |
| --- | --- |
| `ds0_minimal.yaml` | 最小仿真数据集配置，用于早期 sanity、simulator 示例和测试。 |
...
| `double_lane_change_5mps.yaml` | 5 m/s 双移线闭环仿真示例，默认输出到 `output/simulation/double_lane_change_5mps/`。 |
...
单次闭环仿真和 debug 可视化结果目录。每个仿真 request 通常输出到一个独立目录，例如 `output/simulation/double_lane_change_5mps/`。
